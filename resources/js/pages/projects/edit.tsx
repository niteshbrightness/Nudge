...
export default function EditProject({ project, clients }: { project: Project; clients: Pick<Client, 'id' | 'name'>[] }) {
    return (
        <AppLayout breadcrumbs={breadcrumbs(project)}>
            <Head title={`Edit ${project.name}`} />
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <Heading title={`Edit ${project.name}`} description="Assign this project to a client for notification routing." />
                        ...
                        {({ errors, processing }) => (
                            <>
                                <div className="grid gap-2">
                                    ...
                                    <Select name="client_id" defaultValue={project.client_id ? String(project.client_id) : 'none'}>
